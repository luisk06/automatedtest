@analyticQ @analyzing @forms @analyzingImage @complete @formsAnalyzing

Feature: Applying a filter to image question results in forms summary view

	As an user
	I want see the results of an image question
	In order to analyse them

	Scenario Outline: The user filters the results of an image in forms with <numAnswers> answers
		Given that there is a webform app with a "forms" with a "image" question that has <numAnswers> answers
