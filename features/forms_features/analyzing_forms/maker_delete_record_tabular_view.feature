@complete @forms @deleteRecordAnalyzeForm @formsAnalyzing

Feature: Deleting records on tabular view in forms

	As an user
	I want to open the delete mode
	In order of delete records

	Scenario: The user has a forms with 2 answers and want to delete the first
	...
	Scenario: The user has a forms with 2 answers and want to delete all
		Given that there is a webform app with a "forms" with a "multiple choice" question that has 2 answers
			And the user is logged in
			And the user opens the just created app
			And the user opens the "webform" board
	...
	Scenario: The user has a forms with 2 answers, opened the delete mode but want to close it without delete records
		Given that there is a webform app with a "forms" with a "multiple choice" question that has 2 answers
			And the user is logged in
			And the user opens the just created app
			And the user opens the "webform" board
