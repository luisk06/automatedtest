@taking @complete @numeric @numericTakerforms @forms @formsTaking

Feature: Answering numeric question in forms

	As as user
	I want to answer a numeric question
	In order to express my answer with a specific number

	Scenario Outline: The user answers numeric question with <name> option as <answer> answer in forms
	...
	Scenario Outline: The user answers numeric question with <name> option as <answer> answer and decimals option checked on forms
		Given the user has an app
			And that the user has a "forms" with a numeric question with <name> option and allow decimals checked
		When the user take the qrvey
			And the user writes <answer> in the numeric input
			And the user clicks outside the input numeric field
			And the user <formatedOnFocus> the numeric input on "forms"
			And the <formatedOnFocus> input has <formatedAns> as answer on "forms"
			And the user clicks the Ok button
		Then the user should jump to the finished qrvey page
