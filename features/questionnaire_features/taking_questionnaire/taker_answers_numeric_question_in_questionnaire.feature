@taking @todo @deprecated @numeric @numericTakerQuestionnaire @questionnaire @questionnaireTaking

Feature: The user answers numeric question in questionnaire

	As as user
	I want to answer a numeric question
	In order to express my answer with a specific number

	Scenario Outline: The user answers numeric question with <name> option as <answer> answer in questionnaire
		Given the user has an app
			And the user has a "questionnaire" with a "numeric_<name>" question
		When the user take the qrvey
			And the user writes <answer> in the numeric input
			And the user clicks outside the input numeric field
			And numeric input has <formatedAns> as <name> answer
	...
	Scenario Outline: The user answers numeric question with <name> option as <answer> answer and decimals option checked on questionnaire
		Given the user has an app
			And that the user has a "questionnaire" with a numeric question with <name> option and allow decimals checked
		When the user take the qrvey
			And the user writes <answer> in the numeric input
			And the user clicks outside the input numeric field
			And the user <formatedOnFocus> the numeric input on "questionnaire"
			And the <formatedOnFocus> input has <formatedAns> as answer on "questionnaire"
			And the user clicks the Ok button
			And the user clicks on the Submit button
		Then the user should jump to the finished qrvey page
