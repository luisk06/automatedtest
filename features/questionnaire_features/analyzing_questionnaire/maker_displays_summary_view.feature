@todo @deprecated @analyticQ @analyzing @date @questionnaire @questionnaireAnalyzing @questionnaireAnalyzingDate

Feature: The user displays summary view a date question results in questionnaire

	As an user
	I want see the results of a date question
	In order to analyse the open responses of the users

	Scenario Outline: The user filters the results of a date question with <numAnswers> answers
		Given that there is a webform app with a "questionnaire" with a "date" question that has <numAnswers> answers with the following dates:
				| dates    |
				| 01/01/01 |
				| 02/02/02 |
				| 03/03/03 |
