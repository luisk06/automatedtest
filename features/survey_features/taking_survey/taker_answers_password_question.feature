@taking @complete @password @survey @surveyTaking @surveyPassword

Feature: Answering password question in survey

	As as user
	I want to answer a password question
	In order to express my answer with a password in the question

	Scenario: The user answers password question in survey
		Given the user has an app
			And that the user has a "survey" with "password" question left
		When the user take the qrvey
			And the user selects answers in "password" question
			And the user clicks the Ok button
		Then the user should jump to the finished qrvey page

	Scenario: The user answers password question with confirm in survey
		Given the user has an app
			And that the user has a "survey" with "password_confirm" question left
		When the user take the qrvey
