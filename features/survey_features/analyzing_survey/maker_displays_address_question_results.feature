@analyticQ @analyzing @survey @analyzingSurveyAddress @complete @surveyAnalyzing

Feature: Excluding answres on address question results in survey individual records

	As an user
	I want see the results of an address question
	In order to analyse them

	Scenario: The user excludes answers from a survey that has a address question with 34 answers
		Given that there is a webform app with a "survey" with a "address" question that has 34 answers
