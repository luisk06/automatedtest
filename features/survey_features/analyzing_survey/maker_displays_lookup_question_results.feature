...
Feature: Applying filters to a Look up question results on survey summary view

	As an user
	I want see the results of a look-up question on summary view
	In order to analyse them by looking at the differents predefined options that have been answered

	@analyzingLookUpQuestionResults
	Scenario Outline: The user applies a <filterType> filter to look up question in survey summary view
		Given that there is a webform app with a "survey" with a "<lookupType>" question that has 20 answers
