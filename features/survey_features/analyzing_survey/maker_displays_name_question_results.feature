@analiticQ @analyzing @complete @survey @analyzingNameSurvey @surveyAnalyzing

Feature: Excluding answres on name question results in survey individual records

	As an user
	I want see the results of an name question
	In order to analyse them

	Scenario: The user excludes answers from a survey that has a name question with 34 answers
		Given that there is a webform app with a "survey" with a "name" question that has 34 answers
