@analyticQ @analyzing @survey @surveyAnalyzing @analyzingSurveyPhone @complete

Feature: Excluding answres on phone number question results on survey individual records

	As an user
	I want see the results of an phone question
	In order to analyse them

	Scenario: The user excludes answers from a survey that has a phone question with 34 answers
		Given that there is a webform app with a "survey" with a "phone" question that has 34 answers
