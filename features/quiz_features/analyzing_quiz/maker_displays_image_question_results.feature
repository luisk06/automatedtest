@analyticQ @analyzing @quiz @analyzingImage @complete @quizAnalyzing

Feature: Applying a filter to image question results in quiz

	As an user
	I want see the results of an image question
	In order to analyse them

	@jenkinsRunningServer
	Scenario Outline: The user filters the results of a image question in quiz with <numAnswers> answers
		Given that there is a webform app with a "quiz" with a "image" question that has <numAnswers> answers
