@analyticQ @analyzing @todo @date @quiz @quizAnalyzing @quizAnalyzingDate

Feature: Applying a filter to a date question results in quiz

	As an user
	I want see the results of a date question
	In order to analyse the open responses of the users

	Scenario Outline: The user filters the results of a date question in quiz with <numAnswers> answers
		Given that there is a webform app with a "quiz" with a "date" question that has <numAnswers> answers with the following dates:
				| dates    |
				| 01/01/01 |
				| 02/02/02 |
				| 03/03/03 |
		...
		Then the "<filteredAnswer>" date answer filter should appear in the histogram filters
			...
			And the individual responses should be <afterFilterAnswers>
