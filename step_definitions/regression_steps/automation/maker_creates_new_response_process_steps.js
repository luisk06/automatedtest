...
module.exports = function () {
	...
	When(/^the user clicks on AddCondition$/, function (cb) {
		webpage.waits(300);
		maker.findsXpath('//span[contains(text(),"+ Add Condition")]').click().then(cb);
	});
	...
	When(/^the user add the first field$/, function (cb) {
		maker.finds('.spec-automatiq-add-field').click();
		maker.findsAll('.spec-automatiq-add-field li').get(1).click().then(cb);
	});
