...
module.exports = function() {
	...
	When(/^the user writes the long text question with less than (\d+) chars$/, function(arg1, cb) {
		questionTitle = rand.getText(+arg1 - 20);
		qrvey.fillTextFieldQuestion(questionTitle).then(cb);
	});

	When(/^the user writes the long text question with more than (\d+) chars$/, function(arg1, cb) {
		questionTitle = rand.getText(+arg1 + 30);
		qrvey.fillTextFieldQuestion(questionTitle).then(cb);
	});
	...
	Then(/^the long text question should be saved with the first (\d+) chars the user wrote$/, function(arg1, cb) {
		var ele = element(by.css('.spec-question-title')),
			isPresent = EC.presenceOf(ele),
			_outputText = null;
