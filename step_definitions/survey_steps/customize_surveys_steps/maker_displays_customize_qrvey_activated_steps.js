...
module.exports = function() {
	...
	When(/^the user try clicks on Load Theme button$/, function(cb) {
		maker.finds('.spec_load_style_button').click().then(function(){
	...
	When(/^the user try clicks on Upload Logo button$/, function(cb) {
		maker.finds('.spec_customize_field_upload').click().then(function(){
	...
	When(/^the user try writes another Label text$/, function(cb) {
		maker.finds('. spec-customize-input-label').click().then(function(){
	...
	When(/^the user try writes another Title$/, function(cb) {
		maker.finds('.spec-customize-input-title').click().then(function(){
			throw new Error(new Error('Background color was changed'));
		}, function(){
			cb();
		});
	...
	When(/^the user try change background color$/, function(cb) {
		maker.finds('.spec_customize_color_option_background').click().then(function(){
			throw new Error(new Error('Background color was changed'));
		}, function(){
			cb();
		});
	...
	When(/^the user try selects the "([^"]*)" position$/, function(position, cb) {
		maker.finds('.spec-tab-'+position).click().then(function(){
	...
	When(/^user try enters "([^"]*)" as title$/, function(title, cb) {
		_title = title;
		maker.finds('.spec-customize-input-title').click().then(
		function(){
