...
module.exports = function() {
	...
	When(/^the user go to text categorization dashboard$/, function(cb) {
		maker.findsAll('.spec_text_categorization_button').first().click().then(cb);
	});
	...
	When(/^the user type the name and description rule$/, function(cb) {
		...
		maker.finds('.spec-input-new-process-name').sendKeys(rand.getSentence(5));
		maker.finds('.spec-input-new-process-description').sendKeys(rand.getSentence(8)).then(cb);
	});
	...
	When(/^the user opens the Category Block$/, function(cb) {
		maker.findsAll('.rule-block').get(1).click().then(cb);
	});

	When(/^the user adds (\d+) new tag$/, function(num, cb) {
		if (num == 1) {
			maker.finds('.tags .tags input.input').sendKeys(rand.getText(5)).then(cb);
		} else if (num > 1) {

			async.times(num, function(item, next) {
				maker.finds('.tags .tags input.input').sendKeys(rand.getText(5));
				maker.finds('.tags .tags input.input').sendKeys(protractor.Key.TAB).then(function(){ next(); });
			}, function() {
				cb();
			});
	...
	When(/^the user adds (\d+) new include tag$/, function(num, cb) {
		if (num == 1) {
			maker.finds('.include .tags input.input').sendKeys(rand.getText(5)).then(cb);
		} else if (num > 1) {
			async.times(num, function(item, next) {
				maker.finds('.include .tags input.input').sendKeys(rand.getText(5));
				maker.finds('.include .tags input.input').sendKeys(protractor.Key.TAB).then(function(){ next(); });
			}, function() {
				cb();
			});
		}
	...
	When(/^the user adds (\d+) new exclude tag$/, function(num, cb) {
		if (num == 1) {
			maker.finds('.exclude .tags input.input').sendKeys(rand.getText(5)).then(cb);
		} else if (num > 1) {
			async.times(num, function(item, next) {
				maker.finds('.exclude .tags input.input').sendKeys(rand.getText(5));
				maker.finds('.exclude .tags input.input').sendKeys(protractor.Key.TAB).then(function(){ next(); });
			}, function() {
				cb();
			});
		}
