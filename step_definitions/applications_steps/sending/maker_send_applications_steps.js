...
module.exports = function () {
	...
	When(/^the user go to the app dashboard$/, function (cb) {
		maker.findsAll('.tab-container.tacenter .tab').get(0).click().then(cb);
	});

	When(/^the user clicks on the application menu$/, function (cb) {
		maker.finds('.drop').click().then(cb);
	});

	When(/^the user writes an valid email$/, function (cb) {
		maker.finds('.contacts_list input').sendKeys(configer.get('username'));
	...
	When(/^the user clicks on the send option$/, function (cb) {
		maker.findsAll('.drop .options span').get(0).click().then(cb);
	});
	...
	When(/^the user writes an invalid email$/, function (cb) {
		maker.finds('.contacts_list input').sendKeys(rand.getEmail());
		element(by.model('name')).click();
		webpage.waits(2000).then(function () {
			cb();
		}).then(cb);
