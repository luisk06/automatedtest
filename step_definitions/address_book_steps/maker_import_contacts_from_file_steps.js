...
module.exports = function() {
	...
	When(/^the user selects the filename$/, function(cb) {
		...
		maker.finds('.spec-addressbook-contacts-upload').sendKeys(absolutePath).getAttribute('value').then(function(_text) {
	...
	When(/^the user clicks on confirm button$/, function(cb) {
		maker.finds('.spec-addressbook-contacts-upload-confirm').click().then(cb);
	});
